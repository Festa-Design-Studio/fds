    ...
    @foreach($certificationsData as $certification)
      <div class="flex items-center gap-4">
        @if(isset($certification['logo']) && $certification['logo'])
          ...
          @endphp
          <img class="w-10 h-10" src="{{ $logoUrl }}" alt="{{ $certification['institution'] ?? 'Institution' }}" class="w-10 h-10 object-contain">
        @else
        ...
        <div>
          <p class="text-body-sm text-the-end-700">
            ...
            @if(isset($certification['institution']))
              — <span class="text-chicken-comb-600">{{ $certification['institution'] }}</span>
            @endif
          </p>
        </div>
      </div>
