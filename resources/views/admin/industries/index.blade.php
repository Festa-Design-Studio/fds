      ...
      <tbody class="divide-y divide-white-smoke-200">
        @forelse($industries as $industry)
          <tr class="hover:bg-white-smoke-50">
            <td class="py-3 px-4">{{ $industry->id }}</td>
            <td class="py-3 px-4">{{ $industry->name }}</td>
            <td class="py-3 px-4">{{ $industry->slug }}</td>
            <td class="py-3 px-4">{{ $industry->projects_count }}</td>
            <td class="py-3 px-4 flex space-x-2">
              <a href="{{ route('admin.industries.edit', $industry->id) }}" class="px-3 py-1 bg-pepper-green-100 text-pepper-green-700 rounded hover:bg-pepper-green-200">
                Edit
              </a>
              <form action="{{ route('admin.industries.destroy', $industry->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this industry?');">
                @csrf
                @method('DELETE')
        ...
        @empty
          <tr>
            <td colspan="5" class="py-6 px-4 text-center text-the-end-500">No industries found.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
