...
@section('title', 'Edit Content Section - Design for Good')

@section('header_title', 'Edit Content Section')
...
<div class="p-8 max-w-4xl mx-auto bg-white-smoke-100 rounded-lg shadow-sm">
  ...
  <form action="{{ route('admin.about.design-for-good.update', $designForGoodContent) }}" method="POST">
    @csrf
    @method('PUT')
    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <select name="section_key" id="section_key" required class="mt-1 block w-full rounded-md border-white-smoke-300 shadow-sm focus:border-apocalyptic-orange-500 focus:ring-apocalyptic-orange-500 text-body text-the-end-800">
            @foreach($availableSections as $key => $name)
              <option value="{{ $key }}" {{ old('section_key', $designForGoodContent->section_key) === $key ? 'selected' : '' }}>{{ $name }}</option>
            @endforeach
          </select>
          @error('section_key')
        ...
        <div>
          ...
          <input type="number" name="display_order" id="display_order" value="{{ old('display_order', $designForGoodContent->display_order) }}" min="0" required class="mt-1 block w-full rounded-md border-white-smoke-300 shadow-sm focus:border-apocalyptic-orange-500 focus:ring-apocalyptic-orange-500 text-body text-the-end-800">
          @error('display_order')
            <p class="mt-1 text-sm text-apocalyptic-orange-500">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="md:col-span-2">
          ...
          <input type="text" name="title" id="title" value="{{ old('title', $designForGoodContent->title) }}" required class="mt-1 block w-full rounded-md border-white-smoke-300 shadow-sm focus:border-apocalyptic-orange-500 focus:ring-apocalyptic-orange-500 text-body text-the-end-800">
          @error('title')
            <p class="mt-1 text-sm text-apocalyptic-orange-500">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="md:col-span-2">
          ...
          <input type="text" name="subtitle" id="subtitle" value="{{ old('subtitle', $designForGoodContent->subtitle) }}" class="mt-1 block w-full rounded-md border-white-smoke-300 shadow-sm focus:border-apocalyptic-orange-500 focus:ring-apocalyptic-orange-500 text-body text-the-end-800">
          @error('subtitle')
            <p class="mt-1 text-sm text-apocalyptic-orange-500">{{ $message }}</p>
          @enderror
        </div>
    ...
    <div class="mt-8 space-y-6">
      ...
      <div>
        ...
        <textarea name="description" id="description" rows="4" class="mt-1 block w-full rounded-md border-white-smoke-300 shadow-sm focus:border-apocalyptic-orange-500 focus:ring-apocalyptic-orange-500 text-body text-the-end-800">{{ old('description', $designForGoodContent->description) }}</textarea>
        @error('description')
          <p class="mt-1 text-sm text-apocalyptic-orange-500">{{ $message }}</p>
        @enderror
      </div>
    ...
    <div class="mt-8 space-y-6">
      ...
      <div>
        <label class="flex items-center">
          <input type="checkbox" name="is_active" value="1" {{ old('is_active', $designForGoodContent->is_active) ? 'checked' : '' }} class="rounded border-white-smoke-300 text-apocalyptic-orange-600 shadow-sm focus:ring-apocalyptic-orange-500">
