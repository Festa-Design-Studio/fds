...
@section('title', 'Edit Sector - Festa Design Studio')

@section('header_title', 'Edit Sector')
  ...
  <form method="POST" action="{{ route('admin.sectors.update', $sector->id) }}">
    ...
    <div class="space-y-4">
      <div>
        ...
        <input type="text" id="name" name="name" value="{{ old('name', $sector->name) }}" class="w-full h-10 px-4 py-2 bg-white-smoke-50 border border-the-end-200 rounded-full text-the-end-900 placeholder-the-end-400 focus:ring-1 focus:ring-chicken-comb-300 focus:border-chicken-comb-600 hover:bg-chicken-comb-50 hover:border-chicken-comb-600/50 disabled:bg-white-smoke-100 disabled:text-the-end-300 disabled:cursor-not-allowed @error('name') border-chicken-comb-600 @enderror" placeholder="Enter sector name" required>
        @error('name')
      ...
      <div class="pt-4 flex space-x-4">
        ...
        <form action="{{ route('admin.sectors.destroy', $sector->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this sector?');" class="inline">
          @csrf
          @method('DELETE')
