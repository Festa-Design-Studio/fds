...
@section('title', 'Edit SDG Alignment - Festa Design Studio')

@section('header_title', 'Edit SDG Alignment')
...
<div class="p-8 max-w-2xl mx-auto bg-white-smoke-100 rounded-lg shadow-sm">
  <form method="POST" action="{{ route('admin.sdg-alignments.update', $sdgAlignment->id) }}">
    @csrf
    @method('PUT')
    <div class="space-y-4">
      <div>
        ...
        <input type="text" id="name" name="name" value="{{ old('name', $sdgAlignment->name) }}" class="w-full h-10 px-4 py-2 bg-white-smoke-50 border border-the-end-200 rounded-full text-the-end-900 placeholder-the-end-400 focus:ring-1 focus:ring-chicken-comb-300 focus:border-chicken-comb-600 hover:bg-chicken-comb-50 hover:border-chicken-comb-600/50 disabled:bg-white-smoke-100 disabled:text-the-end-300 disabled:cursor-not-allowed @error('name') border-chicken-comb-600 @enderror" placeholder="Enter SDG alignment name" required>
        @error('name')
      ...
      <div>
        ...
        <input type="text" id="code" name="code" value="{{ old('code', $sdgAlignment->code) }}" class="w-full h-10 px-4 py-2 bg-white-smoke-50 border border-the-end-200 rounded-full text-the-end-900 placeholder-the-end-400 focus:ring-1 focus:ring-chicken-comb-300 focus:border-chicken-comb-600 hover:bg-chicken-comb-50 hover:border-chicken-comb-600/50 disabled:bg-white-smoke-100 disabled:text-the-end-300 disabled:cursor-not-allowed @error('code') border-chicken-comb-600 @enderror" placeholder="Enter SDG code (optional)">
        @error('code')
          <p class="mt-1 text-chicken-comb-600 text-sm">{{ $message }}</p>
        @enderror
      </div>
      ...
      <div class="pt-4 flex space-x-4">
        ...
        <form action="{{ route('admin.sdg-alignments.destroy', $sdgAlignment->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this SDG alignment?');" class="inline">
          @csrf
          @method('DELETE')
          <button type="submit" class="lg:w-auto md:w-auto w-full px-6 py-3 text-body-lg h-[48px] border-2 border-chicken-comb-600/50 text-chicken-comb-700 rounded-full hover:bg-chicken-comb-50 active:bg-chicken-comb-100 transition-all duration-150 ease-in-out focus:ring-2 focus:ring-chicken-comb-600 focus:ring-offset-2 inline-flex items-center justify-center">
            Delete
          </button>
        </form>
      </div>
