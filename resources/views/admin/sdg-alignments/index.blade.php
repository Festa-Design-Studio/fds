      ...
      <tbody class="divide-y divide-white-smoke-200">
        @forelse($sdgAlignments as $sdgAlignment)
          <tr class="hover:bg-white-smoke-50">
            <td class="py-3 px-4">{{ $sdgAlignment->id }}</td>
            <td class="py-3 px-4">{{ $sdgAlignment->name }}</td>
            <td class="py-3 px-4">{{ $sdgAlignment->code }}</td>
            <td class="py-3 px-4">{{ $sdgAlignment->slug }}</td>
            <td class="py-3 px-4">{{ $sdgAlignment->projects_count }}</td>
            <td class="py-3 px-4 flex space-x-2">
              <a href="{{ route('admin.sdg-alignments.edit', $sdgAlignment->id) }}" class="px-3 py-1 bg-pepper-green-100 text-pepper-green-700 rounded hover:bg-pepper-green-200">
                Edit
              </a>
              <form action="{{ route('admin.sdg-alignments.destroy', $sdgAlignment->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this SDG alignment?');">
                @csrf
                @method('DELETE')
        ...
        @empty
          <tr>
            <td colspan="6" class="py-6 px-4 text-center text-the-end-500">No SDG alignments found.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
