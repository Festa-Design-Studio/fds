...
class FestaRichTextEditor {
    constructor(options = {}) {
        ...
        console.log(
            "🎯 FestaRichTextEditor constructor called with options:",
            this.options
        );
        ...
        if (!this.element) {
            console.error(
                `❌ Element with selector ${this.options.selector} not found`
            );
            return;
        }
    ...
    createEditorContainer() {
        ...
        this.wrapper.className =
            "festa-editor-wrapper border border-white-smoke-300 rounded-lg overflow-hidden";

        // Create toolbar container
        this.toolbarContainer = document.createElement("div");
        this.toolbarContainer.className =
            "festa-editor-toolbar bg-white-smoke-50 border-b border-white-smoke-300 p-2 flex flex-wrap gap-2";

        // Create content container
        this.contentContainer = document.createElement("div");
        this.contentContainer.className =
            "festa-editor-content p-4 min-h-[300px] bg-white focus:outline-none";
    ...
    initTipTap() {
        ...
        if (!initialContent.trim()) {
            this.contentContainer.innerHTML =
                '<p class="text-the-end-400">Start writing your content...</p>';
        }
        ...
        this.contentContainer.addEventListener("focus", () => {
            if (
                this.contentContainer.innerHTML.includes(
        ...
        this.editor = {
            chain: () => ({
                focus: () => ({
                    ...
                    toggleItalic: () => ({
                        run: () => this.execCommand("italic"),
                    }),
                    toggleUnderline: () => ({
                        run: () => this.execCommand("underline"),
                    }),
                    insertContent: (content) => ({
                        run: () => this.insertContent(content),
                    }),
                }),
            }),
    ...
    insertContent(content) {
        ...
        if (
            !this.contentContainer.textContent.trim() ||
            this.contentContainer.innerHTML.includes(
                "Start writing your content..."
            )
        ) {
    ...
    createToolbar() {
        ...
        this.addDivider();
        this.createLinkControls(); // NEW: Add link controls
        this.addDivider();
        ...
        const headingOptions = [
            {
                ...
                value: "h1",
                classes: "text-display font-bold text-pepper-green",
            },
            {
                label: "H1 - Main Title",
                value: "h1",
                classes: "text-h1 font-bold text-pepper-green",
            },
            {
                label: "H2 - Section",
                value: "h2",
                classes: "text-h2 font-bold text-pepper-green",
            },
            {
                label: "H3 - Subsection",
                value: "h3",
                classes: "text-h3 font-semibold text-the-end-900",
            },
            {
                label: "H4 - Point",
                value: "h4",
                classes: "text-h4 font-semibold text-the-end-900",
            },
            {
                label: "H5 - Subpoint",
                value: "h5",
                classes: "text-h5 font-medium text-chicken-comb-600",
            },
            {
                label: "H6 - Detail",
                value: "h6",
                classes: "text-h6 font-medium text-chicken-comb-600",
            },
        ];

        const dropdown = this.createStyledDropdown(
            "Headings",
            headingOptions,
            (option) => {
    ...
    formatHeading(tagName, classes) {
        ...
        if (element && element.parentNode === this.contentContainer) {
            ...
            console.log(
                `✅ Applied heading ${tagName} with classes: ${classes}`
            );
        } else {
        ...
        const paragraphOptions = [
            {
                label: "Body Large",
                value: "large",
                classes: "text-body-lg text-the-end-700",
            },
            {
                label: "Body Regular",
                value: "normal",
                classes: "text-body text-the-end-700",
            },
            {
                label: "Body Small",
                value: "small",
                classes: "text-body-sm text-the-end-600",
            },
        ];
    ...
    // NEW: Add link controls method
    createLinkControls() {
        const linkButton = this.createButton("link", "Add Link", () =>
            this.showLinkDialog()
        );
        this.toolbarContainer.appendChild(linkButton);
    }

    // NEW: Show link dialog
    showLinkDialog() {
        this.contentContainer.focus();
        ...
        if (url && url.trim() !== "") {
            this.createLink(url.trim());
        }
    }

    // NEW: Create link method
    createLink(url) {
        this.contentContainer.focus();
        ...
        link.href = url;
        link.className =
            "text-chicken-comb-600 hover:text-chicken-comb-700 underline";
        link.target = "_blank";
    ...
    createColorControls() {
        ...
        colorPickerBtn.className =
            "px-3 py-1.5 rounded hover:bg-white-smoke-100 flex items-center gap-2";
    ...
    showColorPalette() {
        ...
        this.colorPalette.className =
            "absolute z-10 p-3 bg-white shadow-lg rounded-lg border border-white-smoke-300 mt-1 grid grid-cols-7 gap-1";
        ...
        colorFamilies.forEach((family) => {
            ...
            familyLabel.className =
                "text-xs text-the-end-600 font-medium text-center mb-1";
        ...
        presets.className =
            "col-span-7 mt-2 pt-2 border-t border-white-smoke-200";
        ...
        closeBtn.className =
            "absolute top-2 right-2 text-xs text-the-end-400 hover:text-the-end-700";
        ...
        document.addEventListener("click", (e) => {
            if (
                ...
                !e.target.closest('button[data-action="color"]')
            ) {
                this.colorPalette.classList.add("hidden");
            }
        });
        ...
        const listButtons = [
            {
                icon: "list-ul",
                title: "Bullet List",
                action: () => this.execCommand("insertUnorderedList"),
            },
            {
                icon: "list-ol",
                title: "Numbered List",
                action: () => this.execCommand("insertOrderedList"),
            },
        ];
    ...
    createMediaControls() {
        ...
        const figureBtn = this.createButton(
            "figure",
            "Image with Caption",
            () => {
                // Open figure dialog
        ...
        const videoContainerBtn = this.createButton(
            "video",
    ...
    showFigureDialog() {
        ...
        modal.className =
            "fixed inset-0 bg-the-end-900/50 flex items-center justify-center z-50";

        const dialog = document.createElement("div");
        dialog.className =
            "bg-white rounded-lg shadow-xl w-full max-w-2xl p-6 space-y-4";
        ...
        uploadLabel.className =
            "block text-body-sm font-medium text-the-end-700";
        ...
        uploadInput.className =
            "block w-full px-3 py-2 bg-white-smoke-50 border border-white-smoke-300 rounded text-body-sm";
        ...
        cancelBtn.className =
            "px-4 py-2 text-body-sm bg-white-smoke-50 hover:bg-white-smoke-100 text-the-end-700 rounded";
        ...
        insertBtn.className =
            "px-4 py-2 text-body-sm bg-pepper-green hover:bg-pepper-green-700 text-white rounded";
    ...
    async uploadImage(file) {
        try {
            ...
            if (result.success && result.url) {
                // Insert image at cursor position using insertContent
                ...
                throw new Error(
                    result.message || "Upload failed - no URL returned"
                );
            }
        } catch (error) {
    ...
    insertFigureWithCaption(imageUrl, caption, altText) {
        ...
        const figureHTML = `
            ...
            altText || "Image"
                ...
                ${
                    caption
                        ? `<figcaption class="text-pepper-green-700 text-body-sm text-center">${caption}</figcaption>`
                        : ""
                }
            </figure>
        `;
    ...
    showVideoContainerDialog() {
        ...
        embedTextarea.className =
            "block w-full px-3 py-2 bg-white-smoke-50 border border-white-smoke-300 rounded text-body-sm resize-none";
        embedTextarea.placeholder =
    ...
    insertVideoContainer(embedCode, caption) {
        ...
        const videoContainerHTML = `
            <figure class="bg-white-smoke-50 rounded-lg border border-white-smoke-300 p-4 md:p-8 lg:p-8 mx-auto my-6 w-full">
                ...
                ${
                    caption
                        ? `<figcaption class="mt-4 text-pepper-green-700 text-body-sm text-center">${caption}</figcaption>`
                        : ""
                }
            </figure>
        `;
    ...
    createStyledDropdown(label, options, onSelect, type) {
        ...
        button.type = "button"; // Prevent form submission
        button.className =
            "px-3 py-1.5 rounded hover:bg-white-smoke-100 flex items-center gap-2 border border-white-smoke-300";
        button.innerHTML = `
        ...
        menu.className =
            "absolute z-10 hidden mt-1 py-2 bg-white rounded-lg shadow-lg border border-white-smoke-300 min-w-[220px] max-w-[280px]";

        options.forEach((option) => {
            const item = document.createElement("button");
            item.type = "button"; // Prevent form submission
            item.className =
                "w-full text-left px-4 py-3 hover:bg-white-smoke-50 border-b border-white-smoke-100 last:border-b-0 transition-colors";
        ...
        return `
            <div class="flex flex-col gap-1">
                ...
                <div class="text-xs text-the-end-500">
                    ${option.value.toUpperCase()} • ${option.classes
            .split(" ")
            .slice(0, 2)
            .join(" ")}
                </div>
    ...
    createDropdown(label, options, onSelect) {
        ...
        const menu = document.createElement("div");
        menu.className =
            "absolute z-10 hidden mt-1 py-1 bg-white rounded-lg shadow-lg border border-white-smoke-300 min-w-[180px]";

        options.forEach((option) => {
            const item = document.createElement("button");
            item.type = "button"; // Prevent form submission
            item.className =
                "w-full text-left px-4 py-2 text-body-sm hover:bg-white-smoke-50";
            item.textContent = option.label;
            item.addEventListener("click", (e) => {
                e.preventDefault();
                e.stopPropagation();
...
console.log(
    "✅ IMMEDIATE: FestaRichTextEditor exported successfully:",
    typeof window.FestaRichTextEditor
);
